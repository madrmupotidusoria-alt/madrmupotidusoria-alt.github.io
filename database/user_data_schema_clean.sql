-- SCANORA User Data Management Schema (Clean Version)
...
-- Active User Sessions (only create if not exists)
DO $$
BEGIN
...
-- Enable RLS on user_sessions only if it exists
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'user_sessions') THEN
...
-- Session policies (only if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'user_sessions') THEN
        CREATE POLICY IF NOT EXISTS "Users can manage own sessions" ON user_sessions
            FOR ALL USING (auth.uid() = user_id);
    END IF;
END $$;
